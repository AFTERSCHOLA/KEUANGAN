...
| AF2 | `tests/flow-simulation.spec.js` clicks non-existent `"Pilih peran Admin/Trainer"`/`"Ganti Peran"` buttons (legacy soft-login picker) | **Re-confirmed** — soft-login picker removed in M-AUTH.5 (`PRODUCTION_MILESTONES.md:94`) | `tests/flow-simulation.spec.js:194,206-207,251-252` (note: `:194` citation predates the 2026-09-02 trim of `flow-simulation.spec.js` to 61 lines; the `206-207`/`251-252` lines still exist and are the actionable citations — see HY.4.3 doc-typo entry) |
