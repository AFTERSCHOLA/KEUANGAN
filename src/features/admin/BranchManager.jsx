...
export default function BranchManager() {
  ...
  const [cabang, setCabang] = useState(() => readCached('cabang'))  // baca doang, jangan write
  ...
  setSaving(true)
  try {
    // Kirim cabang ke SERVER dulu, tunggu sampai beneran tersimpan,
    // sebelum lanjut bikin akun admin yang butuh cabangId ini valid di DB.
    const cabangPayload = { ...form, kode }
    if (isEdit) {
      cabangPayload.action = 'update'
    }
    const cabangResult = await writeRemote('cabang', cabangPayload)

    if (cabangResult.status === 'forbidden') {
      showError(cabangResult.message || 'Kamu tidak punya izin untuk menyimpan cabang ini.')
      return
    }
    if (cabangResult.status === 'conflict') {
      showError('Data cabang ini sudah berubah di server. Muat ulang halaman sebelum menyimpan lagi.')
      return
    }
    ...
    showError(error?.message || 'Gagal menyimpan cabang.')
  } finally {
    setSaving(false)
  }
}
