...
|---|----------|-------------------|
| D1 | Accounting basis | **Cash basis.** Laba/Rugi = SPP lunas (cash in) − Honor Dibayar (cash out). Accrual figures (Beban Honor, Sisa Kewajiban) displayed as clearly-labeled *memo* rows, never mixed into profit. |
...
### M0 — Scaffold & Split *(Person 1 only — gate for everyone)*
...
### M1 — Data Model & Academic Year *(Persons 2 + 5 lead; 1 supports)*
...
### M2 — Money Correctness *(Person 3 leads; Person 4 joins for exports)*
...
### M3 — Safety & Operator Value *(Person 4 leads)*
...
### M4 — Presentation *(Person 5 leads; everyone fixes their own track's integration bugs)*
...
**R4. Ledger trust.** Payment and attendance aggregates derive from `honorPayments` + `absensi` — never from current entity state. UI never computes "expected honor" by multiplying sessions by a rate at read time *except* inside `finance.js` where it's defined once.
...
Facade of the workload: each milestone decomposes into a **strictly-ordered chain of microtasks** conforming to Part 6. The sequence *is* the contract — do not start microtask N+1 until N's VERIFY passes. (Person assignments in parentheses refer to Part 5.)
